...
class RegexGeneratorApp:
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(main_frame, text="Iniciar con (^)", variable=self.match_start, command=self.update_regex).grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Checkbutton(main_frame, text="Terminar con ($)", variable=self.match_end, command=self.update_regex).grid(row=2, column=0, sticky=tk.W, pady=5)
        ttk.Checkbutton(main_frame, text="Dígitos (0-9)", variable=self.digits, command=self.update_regex).grid(row=3, column=0, sticky=tk.W, pady=5)
        ttk.Checkbutton(main_frame, text="Letras (a-z, A-Z)", variable=self.letters, command=self.update_regex).grid(row=4, column=0, sticky=tk.W, pady=5)
        ttk.Checkbutton(main_frame, text="Caracteres especiales", variable=self.special_chars, command=self.update_regex).grid(row=5, column=0, sticky=tk.W, pady=5)
        
        # Entrada para patrón personalizado
        ttk.Label(main_frame, text="Patrón personalizado:").grid(row=6, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.custom_pattern).grid(row=7, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        self.custom_pattern.trace("w", self.update_regex)
        
        # Entradas para longitud
        ttk.Label(main_frame, text="Longitud mínima:").grid(row=8, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.min_length, width=10).grid(row=8, column=1, sticky=tk.W, pady=5)
        self.min_length.trace("w", self.update_regex)
        
        ttk.Label(main_frame, text="Longitud máxima:").grid(row=9, column=0, sticky=tk.W, pady=5)
        ttk.Entry(main_frame, textvariable=self.max_length, width=10).grid(row=9, column=1, sticky=tk.W, pady=5)
        self.max_length.trace("w", self.update_regex)
        
        # Área de visualización del regex
        ttk.Label(main_frame, text="Expresión Regular Generada:").grid(row=10, column=0, sticky=tk.W, pady=10)
        self.regex_display = scrolledtext.ScrolledText(main_frame, height=4, width=50, wrap=tk.WORD)
        self.regex_display.grid(row=11, column=0, columnspan=2, pady=5)
        self.regex_display.config(state='disabled')
        
        # Botón para copiar
        ttk.Button(main_frame, text="Copiar al Portapapeles", command=self.copy_to_clipboard).grid(row=12, column=0, columnspan=2, pady=10)
    ...
    def update_regex(self, *args):
        ...
        if self.custom_pattern.get():
            char_set.append(self.custom_pattern.get())
